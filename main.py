...
with st.sidebar:
    ...
    if st.button("Load Logs and Convert to CSV"):
...
if csv_file:
    ...
    if st.button("Plot Average Total Latency Over Scope"):
    ...
    if st.button("Plot Hop Incremental Latency"):
        if hop_number:
    ...
    if st.checkbox("Suggest High Latency Periods"):
        if 'df_total_filtered_latency' in locals():
            ...
            st.warning("Please plot the average total latency over scope first.")

    if st.checkbox("Visualize High Latency Periods"):
        if 'high_latency_periods' in locals() and not high_latency_periods.empty:
